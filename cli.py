...
def cli(dbpath, dedup, url, token, export, config, v, filename):
    ...
    if not export:
        ...
        if url is None:
            raise click.UsageError("The Firefly URL is required in the export mode.")
        if token is None:
            raise click.UsageError("The Firefly token is required in the export mode.")

        exporter = Exporter(logger, db, FireflyClient(logger, url, token), load_config(logger, config))
