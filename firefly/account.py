...
class AccountExporter:
                ...
                if ff_a is None:
                    ...
                    if config is not None:
                        if ff_a.attributes.active and (not config.active or config.ignore):
                            self.__client.update_account(
                                ff_a.id, AccountUpdate(name=ff_a.attributes.name, active=False, include_net_worth=False)
                            )
        finally:
    ...
    def __to_ff(self, account: Account) -> AccountStore | None:
        ...
        account_type = ShortAccountTypeProperty(config.type if config.type else self.__settings.default_account_type)
        ...
        if account_type == ShortAccountTypeProperty.ASSET:
            ff.account_role = AccountRoleProperty(config.role if config.role else self.__settings.default_account_role)
